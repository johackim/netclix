...
const vodlocker = async (name, season, episode) => {
    ...
    if (!!season && !!episode) {
        const embed = await new Horseman(phantomjs).open(`https://vodlocker.to/embed?referrer=link&t=${nameEncoded}&season=${season}&episode=${episode}`)
        .evaluate(() => {
	...
	embed = await new Horseman(phantomjs).open(`http://vodlocker.to/embed?i=${id}`) .evaluate(() => {
            if (document.getElementById('player_frame')) {
                return document.getElementById('player_frame').innerHTML;
            }
